...
[t,state]=ode45(@dynamics3Dhovercontrollfinal,time,initialValues);
...
for i=1:length(t)
    ...
    %     defining rotation matrix ZXY rotations
    psi=gama(i); %Z
    phi=alpha(i); %X
    theta=beta(i); %Y
    ...
    % quadrotor in fixed frame
    An=R*A + d;
    Bn=R*B + d;
    Cn=R*C + d;
    Dn=R*D + d;
    
    % seperating coordinates of quadrotor frame in fixed frame
    Anx=An(1);
    Any=An(2);
    Anz=An(3);
    Bnx=Bn(1);
